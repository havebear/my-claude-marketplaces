...
2. **检查验证结果**：
   - 检查 subAgent 返回的局部验证结果
   - 局部验证仅要求覆盖该步骤改动直接相关的 lint、类型检查、测试等内容
...
5. **全局验证失败处理**：
   - 如果全局验证失败，主 Agent 负责汇总失败命令和错误信息
...
9. **验证职责边界**：
   ...
   - `ps-exec` 必须在所有步骤完成后负责项目级全局验证
   - 不能因为局部验证通过，就跳过最终的全局 lint、类型检查、测试或构建
